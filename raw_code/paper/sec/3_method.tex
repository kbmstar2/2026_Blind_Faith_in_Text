...
SFT can teach the correct answer, but it does not explicitly contrast the correct answer against the misleading answer that the model actually preferred.
GRPO can reward correct answers, but it must discover useful alternatives through sampling.
In contrast, text-following error mining uses the base model's own failure as the rejected response.
The resulting pair asks the model to reverse a concrete preference it already exhibited under conflict.
...
Corrupted examples provide the main negative signal: a plausible text answer should lose when it conflicts with the image.
Match examples test whether the model can still benefit from reliable text.
Irrelevant and no-text examples test whether the model can answer without overusing unsupported context.
